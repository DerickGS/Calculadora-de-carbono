...
function calcularArvore() {
    ...
    if (!pegadaTotalTexto || pegadaTotalTexto.trim() === "") {
        Swal.fire("Pulou uma etapa", "Por favor, faça o cálculo da pegada de carbono primeiro.", "error");
        return;
    }
    ...
    if (isNaN(pegadaToneladas) || pegadaToneladas <= 0) {
        Swal.fire("Pulou uma etapa", "O cálculo da pegada de carbono está incorreto ou vazio.", "error");
        return;
    }
    ...
    Swal.fire("Árvores Necessárias", `Precisaria plantar cerca de ${arvoresNecessariasArredondado} árvore(s) por passageiro.`, "success");

    // Exibe o resultado no elemento <h4 id="resultadoArvores">
    document.getElementById('resultadoArvores').innerText =
    ...
    document.getElementById('resultadoArvoresGrande').innerText =
...
function calcularCredito() {
    ...
    const resultadoArvores = document.getElementById('resultadoArvores').innerText;
    if (resultadoArvores === '') {
        Swal.fire("Pulou uma etapa", "Por favor, faça o cálculo das árvores primeiro.", "error");
    ...
    if (pegadaCarbono < 1000) {
        ...
        document.getElementById('resultadoCreditoGrande').innerText = 'Abaixo de 1 tonelada por passageiro'; // Mensagem informativa
    } else {
        ...
        document.getElementById('resultadoCreditoGrande').innerText = `${quantidadeCreditos} Crédito(s) de Carbono por passageiro`;
    }

    // Exibe o resultado no h4
    document.getElementById('resultadoCredito').innerHTML = resultadoCredito; // Alterado para innerHTML
    // Exibe a mensagem no swal
    Swal.fire("Créditos de Carbono", `${quantidadeCreditos} Crédito(s) de Carbono por passageiro.`, "success");
}

export {calculoArvoresRealizado, calcularArvore, calcularCredito};